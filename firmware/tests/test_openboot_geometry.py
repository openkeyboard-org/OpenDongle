...
class CleanCheckout(unittest.TestCase):
    def test_geometry_works_with_no_generated_header(self):
        """The generated header must be requested the way OpenBoot DECLARES the
        target -- relative to its firmware directory -- or a fresh clone fails.

        This regressed once and no test caught it: with the header already
        present, make treats an absolute-path request as an already-satisfied
        file and exits 0 without needing a rule, so a developer tree passes
        while a clean checkout fails at PARSE time in both chip Makefiles.
        Deleting the header first is the only way to exercise the real path.
        """
        require_openboot()
        for chip in chip_dirs():
            with self.subTest(chip=chip):
                # NOT skipTest on failure. require_openboot() has already
                # established the submodule is present, so a failure here IS
                # the defect under test -- and on a genuinely clean checkout it
                # is the FIRST call that fails, so skipping would make this
                # whole test pass vacuously exactly when it matters.
                proc = subprocess.run(
                    [sys.executable, "-I", str(GEOMETRY_TOOL),
                     "--openboot", str(OPENBOOT), "--chip", chip,
                     "--transport", "usb", "--board", BOARDS[chip]],
                    capture_output=True, text=True)
                self.assertEqual(
                    0, proc.returncode,
                    f"geometry failed with the header absent or present: "
                    f"{proc.stderr.strip()}")
                header = Path(proc.stdout.split()[4]).parent / "openboot_config.h"
                self.assertTrue(header.is_file(),
                                "geometry did not generate the config header")
                header.unlink()          # regenerated by the run below
                ...
                self.assertEqual(
                    0, again.returncode,
                    f"clean-checkout geometry failed: {again.stderr.strip()}")
                self.assertTrue(header.is_file(), "header was not regenerated")
